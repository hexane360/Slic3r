...
void Print::_make_skirt()
{
    ...
    double min_length_tall = (m_config.ooze_prevention && this->extruders().size() > 1) ? m_config.ooze_skirt_min_length.value : 0;
    double min_length = std::max(m_config.min_skirt_length.value, min_length_tall); // make base skirt at least as long as the tall skirt
    ...
    m_tallSkirt.clear(); // Not sure why only tallSkirt needs this
    ...
    for (int i = n_skirts, extruder_idx = 0; i > 0; -- i) {
        ...
        if (add_to_tall) m_tallSkirt.append(eloop);
